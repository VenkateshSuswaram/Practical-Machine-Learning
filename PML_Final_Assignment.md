...
```r
ErrRateFn = function(TrueValues, PredictedValues) {
    sum(PredictedValues != TrueValues)/length(TrueValues)
}

errRate = ErrRateFn(myTrainingPCA$classe, predictionsA1)
```
...
```
## Warning in `!=.default`(PredictedValues, TrueValues): longer object length
